...
class Character extends MX_Controller
{
	...
	private function getInfo()
	{
		...
		{
			$character_stats = array('maxhealth' => $this->config->item('unknown'));
		}
		...
		{
			$this->secondBar = "mana";
			$this->secondBarValue = $this->config->item('unknown');
		}
		...
		if ($charRecentAchievements)
		{
			...
			foreach ($charRecentAchievements as $key => $achievementData)
			{
				...
				if ($achievementInfo = $this->getDbcModel()->getAchievementInfo($achievementData['achievement']))
				{
					...
					$tooltip = '<div class="wiki-tooltip" id="achievement_tooltip_'.$achievementInfo['id'].'" style="display:none">
									<span class="icon-frame frame-56" style="background-image: url(http://wow.zamimg.com/images/wow/icons/large/'.strtolower($achievementInfo['iconname']).'.jpg);"></span>
									<h3>
										<span class="q0">'.(((int)$achievementInfo['points'] > 0) ? $achievementInfo['points'] . ' ' . $this->config->item('points') : '').'</span>
										'.str_replace("'", "&prime;", $achievementInfo['name']).'
									</h3>
									<span class="color-tooltip-yellow">'.str_replace("'", "&prime;", $achievementInfo['description']).'</span>
								</div>';
					
					$achievementInfo['name_link'] = $tooltip . '<a href="http://wotlk.openwow.com/achievement='.$achievementInfo['id'].'" target="_blank" data-tip-achievement="'.$achievementInfo['id'].'">'.str_replace("'", "&prime;", $achievementInfo['name']).'</a>';
					...
					if ((int)$achievementInfo['points'] > 0)
					{
						...
						//Prepare the text
						$achievementInfo['text'] = str_replace($search, $replace, $this->config->item('achievement_text_with_points'));
						unset($search, $replace);
					}
					else
					{
						$achievementInfo['text'] = str_replace('[NAME]', $achievementInfo['name_link'], $this->config->item('achievement_text'));
					}
	...
	{
		$CacheKey = "character_" . ($this->realm ? $this->realm : 0) . "_" . $this->id;
		...
		else
		{
			if($this->armory_model->characterExists())
			{
				...
				$data = array(
					"module" => "default", 
					"headline" => "<span style='cursor:pointer;' data-tip='".$this->config->item('view_profile')."' onClick='window.location=\"".$this->template->page_url."profile/".$this->account."\"'>".$this->accountName."</span> &rarr; ".$this->name,
					"content" => $character
				);
				...
				{
					// Cache for 30 min
					$this->cache->save($CacheKey, array('page' => $page, 'name' => $this->name), 60*30);
				}
			}
			else
			{
	...
	{
		$this->template->setTitle($this->config->item('character_not_found'));

		$data = array(
			"module" => "default", 
			"headline" => $this->config->item('doesnt_exist'), 
			"content" => "<center style='margin:10px;font-weight:bold;'>".$this->config->item('doesnt_exist_long')."</center>"
		);
	...
	private function getSpecsTable()
	{
		...
		for ($spec = 0; $spec < $this->talentSpecsInfo['speccount']; $spec++)
		{
			//defaults
			$table[$spec] = array(
				'title'		=> $this->config->item('undetermined'),
				'icon'		=> false,
				'points'	=> '0/0/0',
	...
	private function getGlyphTable($spec)
	{
		...
		while (count($table['minor']) < 3)
			$table['minor'][] = array('id' => 0, 'spellid' => 0, 'name' => $this->config->item('empty_glyph'), 'icon' => 'inventoryslot_empty');
			
		while (count($table['major']) < 3)
			$table['major'][] = array('id' => 0, 'spellid' => 0, 'name' => $this->config->item('empty_glyph'), 'icon' => 'inventoryslot_empty');
			
		while (count($table['prime']) < 3)
			$table['prime'][] = array('id' => 0, 'spellid' => 0, 'name' => $this->config->item('empty_glyph'), 'icon' => 'inventoryslot_empty');
