...
class Spell_data extends MX_Controller
{
	...
	{
		$cache = $this->cache->get("spells/spell_data_".($realm ? $realm : 0)."_".$spell);
		...
		else
		{
			...
			if ($info['cacheIt'])
				$this->cache->save("spells/spell_data_".($realm ? $realm : 0)."_".$spell, array('data' => $info['data']), strtotime('1 month', 0));
