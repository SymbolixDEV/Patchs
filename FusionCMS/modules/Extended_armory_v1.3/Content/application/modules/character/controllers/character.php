...
class Character extends MX_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model("wotlk_talents_model");
		$this->load->model("cata_talents_model");
		
		$this->load->model("wotlk_glyphs_model");
		$this->load->model("cata_glyphs_model");
	...
	private function getTalentsModel()
	{
		if ($this->isCataclysm())
		{
			$this->cata_talents_model->setBuild($this->getEmulatorBuild());
			return $this->cata_talents_model;
		}
		
		//default
		//wotlk 3.5.5a
		return $this->wotlk_talents_model;
	}
	
	private function getGlyphsModel()
	{
		if ($this->isCataclysm())
		{
			$this->cata_glyphs_model->setBuild($this->getEmulatorBuild());
			return $this->cata_glyphs_model;
		}
		
		//default
		//wotlk 3.5.5a
		return $this->wotlk_glyphs_model;
	}
	...
	private function getTalentInfo()
	{
		...
		$this->talentTabs = $this->getTalentsModel()->getTalentTabs($tClass);
		...
		if (!empty($this->talentTabs))
		{
			...
			{
				$this->talentTabs[$key]['talents'] = $this->getTalentsModel()->getTalentsForTab($tab['id']);
			}
		}
	...
	private function getGlyphsTableTrinity($data)
	{
		...
		for ($i = 1; $i <= $glyphs; $i++)
		{
			...
			if ((int)$glyphId > 0)
				$temp[] = $this->getGlyphsModel()->getGlyphInfo($glyphId);
			else
				$temp[] = false;
		}
	...
	private function getGlyphsTableMangos($data)
	{
		...
		{
			if ((int)$row['glyph'] > 0)
				$temp[] = $this->getGlyphsModel()->getGlyphInfo($row['glyph']);
			else
				$temp[] = false;
		}
		unset($data, $key, $row);
	...
	{
		if ($icon = $this->getTalentsModel()->getSpellIcon($spell))
