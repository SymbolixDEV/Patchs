...
<section id="armory_top">
	<section id="armory_bars">
		<div id="armory_health">Health: <b>{if strlen($stats.maxhealth)}{$stats.maxhealth}{else}Unknown{/if}</b></div>

		<div id="armory_{$secondBar}">{ucfirst($secondBar)}: <b>{if strlen($secondBarValue)}{$secondBarValue}{else}Unknown{/if}</b></div>
	</section>
...
<section id="armory" style="background-image:url({$url}application/images/armory/{$bg}.png)">
	...
	<section id="armory_stats">
		<center id="armory_stats_top">
			{if $has_stats}<a href="javascript:void(0)" onClick="Character.tab('stats', this)" class="armory_current_tab">{lang("attributes", "character")}</a>{/if}
			<a href="javascript:void(0)" onClick="Character.tab('pvp', this)" {if !$has_stats}class="armory_current_tab"{/if}>{lang("pvp", "character")}</a>
		</center>
...
<section id="armory_title" style="margin-top: 10px;">
    <div style=""><h3>Arena Teams</h3></div>
</section>
