var Menu = {
	 ...
	 */
	Links: {
		remove: "ucp/admin/delete/",
		create: "ucp/admin/create/",
		save: "ucp/admin/save/",
		move: "ucp/admin/move/"
	},
	...
	closeGallery: function(callback)
	{
		...
		{
			$('#image-gallery').fadeOut('fast', callback);
		}
		else
		{
	...
	{
		$('#selected-image').val(str);
		
		$('#image-preview').fadeOut('fast');
		
		this.closeGallery(function()
		{
			$('#image-preview').attr('src', $('#image-preview').attr('data-path') + str).bind('load', function()
			{
				$(this).fadeIn('slow');
			});
		});
