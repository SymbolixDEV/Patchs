...
SpellTargetSelector::SpellTargetSelector(Unit* caster, uint32 spellId) :
    _caster(caster), _spellInfo(sSpellMgr->GetSpellInfo(spellId)/*sSpellMgr->GetSpellForDifficultyFromSpell(sSpellMgr->GetSpellInfo(spellId), caster)*/)
{
    sLog->outError(LOG_FILTER_GENERAL, "SpellTargetSelector::SpellTargetSelector maybe has some error, need fix");
    ASSERT(_spellInfo);
}
